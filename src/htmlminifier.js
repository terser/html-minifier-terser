...
import { minify as terser, minify_sync as terser_sync } from 'terser';

import { HTMLParser, endTag } from './htmlparser.js';
import TokenChain from './tokenchain.js';
import { replaceAsync, replaceGen, yieldCall } from './utils.js';
...
function* cleanAttributeValue(tag, attrName, attrValue, options, attrs) {
  if (isEventAttribute(attrName, options)) {
    ...
    return yield* yieldCall(options.minifyJS, attrValue, true);
    ...
    if (attrValue) {
      ...
      attrValue = yield* yieldCall(options.minifyCSS, attrValue, 'inline');
    }
    return attrValue;
    ...
    attrValue = trimWhitespace(attrValue);
    return yield* yieldCall(options.minifyCSS, attrValue, 'media');
  }
  return attrValue;
}
...
function* cleanConditionalComment(comment, options) {
  return options.processConditionalComments
    ? yield* replaceGen(comment, /^(\[if\s[^\]]+]>)([\s\S]*?)(<!\[endif])$/, function* (match, prefix, text, suffix) {
      return prefix + (yield* minifyHTML(text, options, true)) + suffix;
    })
    : comment;
}
...
function* processScript(text, options, currentAttrs) {
  for (let i = 0, len = currentAttrs.length; i < len; i++) {
    if (currentAttrs[i].name.toLowerCase() === 'type' &&
      ...
      return yield* minifyHTML(text, options);
    }
  }
  return text;
}
...
function* normalizeAttr(attr, attrs, tag, options) {
  ...
  if (attrValue) {
    attrValue = yield* cleanAttributeValue(tag, attrName, attrValue, options, attrs);
  }
  ...
  Object.keys(inputOptions).forEach(function (key) {
    ...
    if (key === 'caseSensitive') {
      ...
      options.minifyCSS = function (text, type) {
        ...
        const output = new CleanCSS(cleanCssOptions).minify(inputCSS);
      ...
      options.minifyJS = function (text, inline) {
        ...
        try {
          const result = terser_sync(code, terserOptions);
...
function* createSortFns(value, options, uidIgnore, uidAttr) {
  ...
  yield* scan(yield* minifyHTML(value, options));
  options.log = log;
  if (attrChains) {
...
function* minifyHTML(value, options, partialMarkup) {
        ...
        if (options.minifyCSS) {
          options.minifyCSS = (function (fn) {
            return function (text, type) {
              ...
              let chunk = fn(text, type);
              // fn may return a Promise (user-supplied async function) or a plain value
              if (chunk && typeof chunk.then === 'function') {
                return chunk.then(c => {
                  ids.forEach(function (id) {
                    c = c.replace(ignoreCSS(id), id);
                  });
                  return c;
                });
              }
  ...
  if ((options.sortAttributes && typeof options.sortAttributes !== 'function') ||
    ...
    yield* createSortFns(value, options, uidIgnore, uidAttr);
  }
  ...
  const parser = new HTMLParser(value, {
    ...
    start: function* (tag, attrs, unary, unarySlash, autoGenerated) {
      ...
      for (let i = attrs.length, isLast = true; --i >= 0;) {
        const normalized = yield* normalizeAttr(attrs[i], attrs, tag, options);
        if (normalized) {
    ...
    chars: function* (text, prevTag, nextTag) {
      ...
      if (options.processScripts && specialContentTags.has(currentTag)) {
        text = yield* processScript(text, options, currentAttrs);
      }
      if (isExecutableScript(currentTag, currentAttrs)) {
        text = yield* yieldCall(options.minifyJS, text);
      }
      if (isStyleSheet(currentTag, currentAttrs)) {
        text = yield* yieldCall(options.minifyCSS, text);
      }
    ...
    comment: function* (text, nonStandard) {
      ...
      if (isConditionalComment(text)) {
        text = prefix + (yield* cleanConditionalComment(text, options)) + suffix;
...
// Shared generator that drives minification. Both minify and minify_sync use this.
function* minify_impl(value, options) {
  const start = Date.now();
  options = processOptions(options || {});
  const result = yield* minifyHTML(value, options);
  options.log('minified in: ' + (Date.now() - start) + 'ms');
  return result;
}

export const minify = async function (value, options) {
  const gen = minify_impl(value, options);
  let step = gen.next();
  while (!step.done) {
...
export const minify_sync = function (value, options) {
  const gen = minify_impl(value, options);
  let step = gen.next();
  while (!step.done) {
    if (step.value && typeof step.value.then === 'function') {
      throw new Error('minify_sync does not support async minifyJS/minifyCSS options — pass a sync function instead.');
    }
...
export default { minify, minify_sync };
