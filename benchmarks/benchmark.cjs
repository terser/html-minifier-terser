...
const table = new Table({
  ...
  colWidths: [
    fileNames.reduce(function (length, fileName) {
...
function generateMarkdownTable() {
  const headers = ['Site', 'Original size *(KB)*', 'HTMLMinifier', 'minimize', 'Will Peavy', 'htmlcompressor.com'];
    ...
    function processFile(site, done) {
      ...
      run(
        ...
        function () {
          ...
          const report = ['[' + fileName + '](' + urls[fileName] + ')', toKb(original.size)];
          for (const name in infos) {
            const info = infos[name];
            display.push(
              [greenSize(info.size), greenSize(info.gzSize), greenSize(info.lzSize), greenSize(info.brSize)].join('\n'),
            );
